...
public class QueryEventHookManagerTest {
  ...
  private static QueryEventHookManager createSut(int nThreads, String... hooks)
      throws Exception {
    if (hooks.length == 0) {
  ...
  public void testHookRegistration() throws Exception {
    final QueryEventHookManager SUT = createSut(1,
    CountingQueryEventHook.class.getCanonicalName(),
        CountingQueryEventHook.class.getCanonicalName());

    final List<QueryEventHook> hooks = SUT.getHooks();
  ...
  public void testHookPostQueryExecuteErrorsDoNotKillExecution() throws Exception {
    ...
    assertEquals(SUT.getHooks().get(0).getClass(), PostQueryErrorEventHook.class);

    final List<Future<QueryEventHook>> futures
        = SUT.executeQueryCompleteHooks(mockQueryCompleteContext);
  ...
  public void testHookPostQueryExecuteInvokedCorrectly() throws Exception {
    ...
    futures = SUT.executeQueryCompleteHooks(mockQueryCompleteContext);
