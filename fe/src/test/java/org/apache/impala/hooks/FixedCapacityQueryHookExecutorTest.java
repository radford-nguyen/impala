...
public class FixedCapacityQueryHookExecutorTest {
  ...
  /**
   * If a query-complete hook task exceeds the timeout value, it
   * should be cancelled
   */
  @Test
  public void testQueryCompleteTimeoutCancelsTask() throws Exception {
    ...
    assertHookTimedOut(f, 1, TimeUnit.SECONDS);

    assertFalse(hookCompleted.get());
    assertTrue(hookInterrupted.get());
  }

  // Encapsulates logic that determines if a hook task timed out.
  private void assertHookTimedOut(Future<QueryEventHook> hookFuture,
                                  long timeout,
  ...
  /**
   * If a query-complete hook task finishes before the timeout, it should
   * not time out.  (duh)
   */
  @Test
  ...
  /**
   * If a hook task is submitted when the hookExecutor queue is full, the
  ...
  public void testHookQueueFullWillRejectSubmission() throws Throwable {
    ...
    try {
      ...
      assertTrue(ee.getCause() instanceof RejectedExecutionException);
    }
  }

  /**
   * If a hook task exceptions, metrics should still be captured.
   */
  @Test
  ...
  /**
   * If a hook task times out, metrics should still be captured.
   */
  @Test
