...
class TestHooks(CustomClusterTestSuite):
  ...
  All test cases in this testsuite share an impala log dir, so keep that in mind
  ...
  LOG_DIR = tempfile.mkdtemp(prefix="test_hooks_", dir=os.getenv("LOG_DIR"))

  def teardown(self):
    # comment this out if you need to see log files to debug fails
  ...
  @CustomClusterTestSuite.with_args(
      ...
      impalad_args="--server_name=server1 "
                   "--sentry_config={0} "
                   "--authorization_provider=sentry "
                   "--query_event_hook_classes={1} "
                   "--lineage_event_log_dir=linwhatev "
                   "--minidump_path={2}"
                   .format(SENTRY_CONFIG_FILE, DUMMY_HOOK, MINIDUMP_PATH),
      catalogd_args="--sentry_config={0} "
                    "--authorization_provider=sentry "
                   "--minidump_path={1}"
                    .format(SENTRY_CONFIG_FILE, MINIDUMP_PATH),
      sentry_config=SENTRY_CONFIG_FILE)
  def test_query_event_hooks_execute(self, unique_role, unique_name):
    """
    ...
    try:
      query("create role {0}".format(unique_role))
      query("grant all on server to role {0}".format(unique_role))
      query("grant role {0} to group {1}".format(unique_role, grp.getgrnam(user).gr_name))
      query("create database {0}".format(unique_db))
      query("grant all on database {0} to role {1}".format(unique_db, unique_role))
      query("create table {0}.recipes "
            "    (recipe_id int, recipe_name string)".format(unique_db))
      query("grant all on table {0}.recipes to role {1}".format(unique_db, unique_role))

      query("INSERT INTO {0}.recipes "
      ...
      impala_client = self.create_impala_client()
      query("grant all on server to role {0}".format(unique_role))
      query("drop table if exists {0}.recipes".format(unique_db))
      query("drop database if exists {0}".format(unique_db))
      query("drop role {0}".format(unique_role))
...
class TestHooksStartupFail(CustomClusterTestSuite):
  ...
  @CustomClusterTestSuite.with_args(
      ...
      impalad_args="--server_name=server1 "
                   ...
                   "--query_event_hook_classes={1} "
                   "--minidump_path={2}"
                   .format(SENTRY_CONFIG_FILE, FAILING_HOOK, MINIDUMP_PATH),
      catalogd_args="--sentry_config={0} "
                    "--server-name=server1 "
                    "--authorization_provider=sentry "
                   "--minidump_path={1}"
                    .format(SENTRY_CONFIG_FILE, MINIDUMP_PATH),
      sentry_config=SENTRY_CONFIG_FILE)
  def test_hook_startup_fail(self, unique_name):
    """
  ...
  @CustomClusterTestSuite.with_args(
      ...
      impalad_args="--server_name=server1 "
                   ...
                   "--query_event_hook_classes={1} "
                   "--minidump_path={2}"
                   .format(SENTRY_CONFIG_FILE, NONEXIST_HOOK, MINIDUMP_PATH),
      catalogd_args="--sentry_config={0} "
                    "--server-name=server1 "
                    "--authorization_provider=sentry "
                   "--minidump_path={1}"
                    .format(SENTRY_CONFIG_FILE, MINIDUMP_PATH),
      sentry_config=SENTRY_CONFIG_FILE)
  def test_hook_instantiation_fail(self, unique_name):
    """
